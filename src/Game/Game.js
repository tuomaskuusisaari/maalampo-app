...
class Game extends Component {
    ...
    render() {
        ...
        let gameStatus = <div className='Game-status'>
            <div>Vuoro: {this.state.turnNo}</div>
            <div>Pareja löydetty: {this.state.pairsFound}</div>
        </div>;

        if (this.state.pairsFound === this.memoryCards.NUM_IMAGES) {
            gameStatus = <div className='Game-status'>
                <div>PELI LÄPÄISTY! Nyt osaat tunnistaa erilaiset maalämpöpumput!</div>
                <div>Käytit {this.state.turnNo - 1} vuoroa</div>
                <div><button onClick={this.onPlayAgain}>Pelaa uudelleen?</button></div></div>;
        }

        return (
            <div className='Game'>
                <header className='Game-header'>
                    <div className='Game-title'>Maalämpöpumppu- muistipeli</div>
                </header>
                <div>
                    {gameStatus}
                </div>
